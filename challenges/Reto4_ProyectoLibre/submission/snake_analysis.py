...
def curvas_largas(base, dqn, figsize=(13, 4.2)):
    ...
    ax = axes[0]
    for res, color, etq in [(base, C_BASE, "sin red objetivo"), (dqn, C_DQN, "con red objetivo")]:
    ...
    ax.set_xlabel("episodio"); ax.set_ylabel("longitud media")
    ...
    ax.set_xlabel("episodio"); ax.set_ylabel("pérdida acumulada")
...
def brecha_train_greedy(res, titulo, ax=None):
    ...
    ax.plot(x, tr_en_x, color=C_BASE, lw=1.6, marker="s", ms=3, label="entrenamiento (ε = 0,1)")
    ax.plot(x, g, color=C_DQN, lw=1.6, marker="o", ms=3, label="greedy (ε = 0)")
    ax.fill_between(x, tr_en_x, g, color=C_DQN, alpha=0.12)
    ax.set_title(titulo, fontsize=10)
    ax.set_xlabel("episodio"); ax.set_ylabel("longitud media")
    ax.grid(alpha=0.25, ls=":"); ax.legend(fontsize=8, frameon=False)
    return ax
...
def grafico_factorial(corridas, figsize=(11, 4.2)):
    ...
    for i, celda in enumerate(orden):
        ...
        ax.scatter(x, vals, color=color, s=45, alpha=0.85, zorder=3, edgecolor="white", linewidth=0.6)
        m = np.mean(vals)
        ax.plot([i - 0.24, i + 0.24], [m, m], color=color, lw=2.4, zorder=4)
        if len(vals) > 1:
            sd = np.std(vals, ddof=1)
            ax.plot([i, i], [m - sd, m + sd], color=color, lw=1.2, alpha=0.6, zorder=2)
    ax.set_xticks(range(len(orden))); ax.set_xticklabels(etiquetas, fontsize=8)
