...
Variantes usadas en el reto:
    ...
    dqn      : baseline + red objetivo + detach  <- aísla el diagnóstico de §5
...
def reset_clean(board):
    ...
    El resetgame() original no restablece snake.dir ni time_since_apple, asi que
    cada partida heredaba la direccion de la anterior y arrancaba con el contador
    a medio correr.
    """
    board.resetgame()
    board.snake.dir = np.array([1.0, 0.0])
    board.time_since_apple = 0


def reubicar_manzana_si_choca(board, max_intentos=100):
    """Corrige la 'manzana imposible': appleclass.eaten() reposiciona al azar sin
    comprobar si la casilla esta ocupada por el cuerpo de la serpiente.

    Cuando cae dentro del cuerpo la manzana es inalcanzable (para llegar habria
    que morderse), y la serpiente muere de inanicion con recompensa -1 por un
    fallo del ambiente, no suyo. La probabilidad crece con la longitud
    (~len/225), asi que castiga al agente justo cuando va ganando. Aqui la
    reubicamos hasta encontrar casilla libre.
    """
...
def evaluate_greedy(net, num_games=30, seed=12345, max_steps=2000, fix_apple=False):
    """Evalua con epsilon = 0. Semilla propia y fija: todas las evaluaciones son
    comparables entre si.

    Esta es la metrica honesta. El autor reporta el promedio de entrenamiento
    (con epsilon = 0.1), lo que subestima su agente en ~50% -- ver seccion 7 del
    notebook.
    """
...
def train(args):
    ...
    for ep in range(1, args.episodes + 1):
        ...
        for _ in range(args.updates):
            ...
            rw = torch.FloatTensor(rw)
            dn = torch.FloatTensor(dn)
            ...
            loss = (
                F.smooth_l1_loss(q_sa, target)
                if args.loss == "huber"
                else F.mse_loss(q_sa, target)
            )
            loss.backward()
            opt.step()
            ep_loss += loss.item()
        ...
        if ep % args.eval_every == 0 or ep == args.episodes:
            g_avg, g_max = evaluate_greedy(
                net, num_games=args.eval_games, fix_apple=args.fix_apple
            )
            ...
            print(
                f"[{args.variant}] ep {ep:5}/{args.episodes}  eps={eps:.3f}  "
                f"train_len={np.mean(lengths):5.2f}  "
                f"GREEDY_len={g_avg:5.2f} (max {g_max:3.0f})  "
                f"loss={ep_loss:8.2f}  transcurrido={el / 60:5.1f}m  "
                f"faltan={eta / 60:5.1f}m",
                flush=True,
            )

        # ---- tope de reloj ----
        # A medida que el agente mejora, sus partidas duran mas pasos y cada episodio se
        # encarece. Sin este tope, el tiempo total es impredecible. Cortar por
        # reloj mantiene ademas la comparacion justa: ambas variantes reciben el
        # MISMO presupuesto de tiempo.
        if args.max_minutes and (time.time() - t0) / 60 >= args.max_minutes:
            print(
                f"\n[{args.variant}] tope de {args.max_minutes} min alcanzado "
                f"en el episodio {ep} "
                f"(de {args.episodes} previstos) -- guardando y terminando.",
                flush=True,
            )
    ...
    print(
        f"[{args.variant}] greedy final sobre 200 partidas: "
        f"media={final_avg:.2f}  max={final_max:.0f}"
    )
...
def main():
    ...
    p.add_argument(
        ...
        type=int,
        default=500,
        help="pasos de gradiente entre sincronizaciones",
    )
    ...
    g.add_argument("--target-net", dest="target_net", action="store_true")
    g.add_argument("--no-target-net", dest="target_net", action="store_false")
    g2 = p.add_mutually_exclusive_group()
    g2.add_argument("--detach", dest="detach", action="store_true")
    ...
    print(
        f"  red objetivo : {args.target_net}"
        + (f"  (sync cada {args.sync_every} pasos)" if args.target_net else "")
    )
    print(f"  detach       : {args.detach}")
    print(f"  perdida      : {args.loss}")
    print(f"  epsilon      : {args.eps}")
    estado_manzana = (
        "corregida" if args.fix_apple else "original (puede caer sobre el cuerpo)"
    )
    print(f"  manzana      : {estado_manzana}")
    print(
        f"  episodios    : {args.episodes}  x {args.games_per_episode} partidas  "
        f"x {args.updates} updates"
    )
