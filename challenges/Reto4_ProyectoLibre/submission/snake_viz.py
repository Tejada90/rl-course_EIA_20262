...
A diferencia del WatchAgent.ipynb del autor (que abre una ventana de Pygame y
necesita que alguien presione ESC para parar), esto:
  - no requiere pantalla ni Pygame,
  - juega un numero fijo de partidas y termina solo,
  - produce un reproductor HTML+JS autocontenido que sobrevive a la exportacion
    del notebook.
"""
...
def record_game(net, gridsize=GRID, seed=7, max_steps=400, epsilon=0.0):
    ...
    Cada fotograma es un dict con el cuerpo de la serpiente, la manzana y los
    marcadores.
    """
...
def animate(frames, gridsize=GRID, titulo="Agente", intervalo=90, figsize=(4.2, 4.6)):
    ...
    def dibujar(i):
        ...
        titulo_txt.set_text(
            f"{titulo}   long: {f['length']}   manzanas: {f['score']}{estado}"
        )
        return patches + [titulo_txt]

    anim = animation.FuncAnimation(
        fig, dibujar, frames=len(frames), interval=intervalo, blit=False
    )
    plt.close(fig)
    return anim
...
def jugar_y_animar(net, titulo="Agente", seed=7, max_steps=400, gridsize=GRID):
    ...
    return HTML(
        animate(frames, gridsize=gridsize, titulo=titulo).to_jshtml(default_mode="once")
    )


def animate_comparison(
    agentes, gridsize=GRID, seed=7, max_steps=400, intervalo=90, alto=3.6, dpi=62
):
    """Anima varios agentes en paralelo dentro de un solo reproductor.

    `agentes` es un dict {titulo: red}. Todos juegan con la MISMA semilla, asi
    que reciben la misma secuencia de manzanas y las diferencias son atribuibles
    a la politica.
    ...
    """
    grabaciones = {
        t: record_game(n, gridsize=gridsize, seed=seed, max_steps=max_steps)
        for t, n in agentes.items()
    }
    n_frames = max(len(f) for f in grabaciones.values())
    for f in grabaciones.values():  # congelar las partidas cortas
        if len(f) < n_frames:
    ...
    def dibujar(i):
        ...
        for idx, (titulo, frames) in enumerate(grabaciones.items()):
            ...
            ap = f["apple"]
            c = mpatches.Circle(
                (ap[0], ap[1]), 0.38, facecolor="#f85149", edgecolor="none"
            )
            ax.add_patch(c)
            estado["patches"][idx].append(c)

            marca = " ✗" if f.get("dead") else ""
            titulos[idx].set_text(
    ...
    anim = animation.FuncAnimation(
        fig, dibujar, frames=n_frames, interval=intervalo, blit=False
    )
    plt.close(fig)
    return anim
